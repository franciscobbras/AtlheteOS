// Supabase Vault access, restricted to service_role — via the SECURITY DEFINER
// helpers public.get_secret / public.upsert_secret.
...
export async function readGoogleSecrets(client: Client): Promise<GoogleSecrets> {
  ...
  if (missing.length) throw new Error(`Vault secrets ausentes: ${missing.join(", ")}`);
